...
def screen_record_connected():
	...
	if(len(deviceDSN)==0):
		...
		print("You have connected some devices.Pls select the one to record the screen..")
...
def screen_record_dsn(i):
	file_name=get_filename(i)
	print(file_name)
	os.system("adb -s "+deviceDSN[i]+" shell screenrecord /sdcard/Screcord_"+file_name+".mp4")
#	time.sleep(5)
	os.system("adb -s "+deviceDSN[i]+" pull /sdcard/Screcord_"+file_name+".mp4")



screen_record_connected()
